...
const AddSizeGroup = () => {
  ...
  const getSizeGroup = async () => {
    try {
      ...
      const res = await api.get(`/sizegroup/${id}`);

      setFormData({
        sizegroup: res.data.sizegroup || ""
      });
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        sizegroup: formData.sizegroup.trim()
      };
      ...
      if (isEdit) {
        ...
        await Swal.fire(
          "Success",
          "Size Group Added Successfully",
          "success"
        );
      }

      navigate("/masters/sizegroup");

    } catch (error) {
      console.error(
        "SIZE GROUP SAVE ERROR:",
        error
      );
